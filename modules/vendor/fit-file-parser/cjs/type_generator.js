...
exports.generateParsedMessages = generateParsedMessages;
...
function generateFitType() {
    ...
    const additionalReferenceProperties = Object.values(fit_js_1.FIT.messages)
        .filter(message => !existingMessageNames.has(message.name))
        .map(message => generateProperty(message.name, typescript_1.default.factory.createTypeReferenceNode(snakeToCamel(`parsed_${message.name}`))));
...
function generateParsedMessages(messages) {
    return typescript_1.default.factory.createInterfaceDeclaration([typescript_1.default.factory.createModifier(typescript_1.default.SyntaxKind.ExportKeyword)], 'ParsedMessages', undefined, undefined, Object.values(messages).map(message => generateArrayProperty(message.name, typescript_1.default.factory.createTypeReferenceNode(snakeToCamel(`parsed_${message.name}`)))));
}
...
function main() {
    ...
    nodes.push(generateParsedMessages(fit_js_1.FIT.messages));
